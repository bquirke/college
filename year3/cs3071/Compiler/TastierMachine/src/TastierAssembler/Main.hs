...
main = do
  ...
  if length args == 2 then do
    ...
    let assemblerSplit = B.split ':' assemblerFile'
    let assemblerInstruct = assemblerSplit !! 0
    let assemblerInstruct1 = B.split '\n' assemblerInstruct
    let assemblerInstruct2 = init assemblerInstruct1
    let charByteString = B.singleton '\n'
    let result = B.intercalate charByteString assemblerInstruct2 

    let assemblerFile = result `B.append` "\n" `B.append` "Call 0 Main\nJmp $END\n" `B.append` assemblerFile'
                        `B.append` "\n$END: Halt\n"
